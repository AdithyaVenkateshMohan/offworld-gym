...
class SequentialMemoryReloadable(SequentialMemory):
    def __init__(self, limit, **kwargs):
        super(SequentialMemoryReloadable, self).__init__(**kwargs)
    ...
    def reload(self, actions, rewards, terminals , observation ,limit_given):
...
def train_withproviedmodel(state_path ):
    ...
    # train the agent
    model ,episode_nr , memory , dqn = dqn.fit(env, callbacks=cbs, action_repetition=1, nb_steps=total_nb_steps, visualize=False,
                verbose=verbose_level, log_interval=log_interval, resume_episode_nr=episode_nr, resume_step_nr=step_nr)

    save_state = True
    return model , callback_save_state , save_state
